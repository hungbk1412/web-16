$.ajax({
    ...
    success : function(result) {
        ...
        questionID = result.question.id; // Đặt questionID trở thành 1 biến toàn cục
        $("#question").attr("result-question", result.question.id)
    },
    error : function(err) {
        console.log(err)
    }
...
$("#no, #yes").on("click", function() {
    $.ajax({
        ...
        success: function(result) {
            console.log(result);
            window.location.href = "/question/" + result.question.id;
        },
        error : function(err) {
        console.log(err)
        }
